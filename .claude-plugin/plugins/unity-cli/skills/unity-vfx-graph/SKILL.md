...
- **New nodes take open space.** Add/duplicate/insert ops refuse to drop a node on an existing one: if the spot is taken (auto or explicit `position`) the node is moved to the nearest free space and the response says `positionAdjusted: true`. Read the returned `position` rather than assuming the one you asked for.
...
- **Short edges over shared nodes.** Do not route one parameter node or operator across the canvas to many consumers. Give far-apart consumers their own parameter canvas node and put cheap operators beside the system they feed; `auto_layout` does both automatically (`duplicateShared`, `splitParameters`), so operator indices can change after it — re-describe before addressing by index.
